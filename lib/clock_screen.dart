...
class _HolidayPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final todayHoliday = holidayOn(now);
    final next = nextHolidayAfter(now);
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(DateFormat.yMMMM(isAr ? 'ar' : 'en_US').format(now),
                  style: TextStyle(color: palette.textMain, fontWeight: FontWeight.w600)),
            ],
          ),
          const Divider(color: Color(0x22FFFFFF)),
          _WeekStrip(now: now, palette: palette, isAr: isAr),
...
class _WeekStrip extends StatelessWidget {
  ...
  final AppPalette palette;
  final bool isAr;
  const _WeekStrip({required this.now, required this.palette, required this.isAr});
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: List.generate(7, (i) {
        ...
        final hasHoliday = holidayOn(d) != null;
        return Column(
          children: [
